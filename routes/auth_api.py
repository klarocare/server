...
async def refresh_token(current_user: User = Depends(AuthHandler.get_current_user)):
    """Get new access token using refresh token"""
    return TokenSchema(
        access_token=AuthHandler.create_access_token(str(current_user.id)),
        refresh_token=AuthHandler.create_refresh_token(str(current_user.id)),
        user=current_user
    )
